...
export class VariantRowRenderer {
  static render(options: VariantRowRenderOptions): string {
    ...
    const closeBtn = bay.state.capabilities.canClose
      ? `<button data-action="closeTab" data-tabid="${esc(bay.metadata.id)}" title="Close"><span class="codicon codicon-close"></span></button>`
      : '';

    return `<div class="bay variant${activeClass}${diffTypeClass}" data-tabid="${esc(bay.metadata.id)}" data-parentid="${parentId}">
